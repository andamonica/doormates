...
const illusData = fetch("api.door-illus.get", {
  method: "POST",
  ...
  .then((response) => {
    if (!response.ok) {
      throw new Error("Failed to fetch door illustrations");
    }
  ...
  .catch((error) => {
    console.error("Error fetching door illustrations:", error);
  });
...
function saveDrawing() {
  ...
  fetch("api.door-illus.save", {
    ...
    .then((data) => {
      console.log("Door illu saved successfully:", data);
    })
    .catch((error) => {
      console.error("Error saving door illu:", error);
    });
